...
parser.add_argument('--model_fusion_type', choices=['concat','bilinear','image','omics'], default='concat', help='model fusion type for finetuning (no pretraining and finetuning in baseline models on same dataset setting (e.g., TCGA-COAD experiments))')
parser.add_argument('--model_pretrain_fusion_type', choices=['concat','bilinear','image','omics'], default='concat', help='model fusion type for pretraining (no pretraining and finetuning in baseline models on same dataset setting (e.g., TCGA-COAD experiments))')
...
parser.add_argument('--feature_folder', default='/media/kevin/DATA2/PathOmics/PathOmics/PathOmics/Extracted_Features', help='extracted feature folder path')
parser.add_argument('--clinical_path', default='/media/kevin/DATA2/PathOmics/PathOmics/PathOmics/coadread_tcga_pan_can_atlas_2018/data_clinical_patient_modified.csv', help='clinical info folder path')
...
parser.add_argument('--save_model_folder_name', default='reproduce_experiments',help='Folder name')
...
parser.add_argument('--load_model_finetune', action='store_true', help='whether in finetuning mode, if so, no need for running pretraining code') # default False (not add)
...
parser.add_argument('--seperate_test', action='store_true', help='Use one fold as test set, do not use it for training or validation') # default True (add)
...
parser.add_argument('--use_GAP_in_pretrain_flag',  action='store_true', help='Whether use GAP in pretrain or not')
# default True (add)
...
#    params = parser.parse_args()
    ...
    path_prefix = '/media/kevin/DATA2/PathOmics/PathOmics/PathOmics'

    load_path = '{}/{}/{}/{}_model_{}_OmicBag_{}_FusionType_{}_OmicType_{}'.format(path_prefix, params.save_model_folder_name, params.experiment_folder_name, params.results_string, params.model_type, params.omic_bag_type, params.model_pretrain_fusion_type, params.omic_modal)
    ...
    if params.load_model_finetune:
        ...
        save_path = '{}/{}/{}/'.format(path_prefix, params.save_model_folder_name, params.experiment_folder_name)
        create_directory(save_path)
        save_path += '/{}_model_{}_OmicBag_{}_FusionType_{}_OmicType_{}'.format(params.experiment_id, params.model_type, params.omic_bag_type, params.model_fusion_type, params.omic_modal)
        create_directory(save_path)
    ...
    # patient_id_list.remove('TCGA-13-0764')
    # patient_id_list.remove('TCGA-36-2533')
    # patient_id_list.remove('TCGA-72-4238')
    # patient_id_list.remove('TCGA-AA-3521')
    # patient_id_list.remove('TCGA-61-2611')
    # patient_id_list.remove('TCGA-61-2610')
    ...
    # print(type(disc_labels))
    # print(disc_labels)
    
    # tmp= torch.Tensor(disc_labels)
    # print(tmp)
    # tmp=torch.Tensor(df_uncensored['label'])
    # df_clinical.to_csv("df_clinical.txt",sep='\t',index=False)
    # return
    ...
    '''
    If k_fold == 1, we don't use k-fold cross-validation. The entire dataset will be splited into train, validation, and test set. Test set is used for model evaluation.


    If k_fold > 1, we use k0fold cross-validation. The entire dataset will be splited into k folds. We used the cross-validated concordance index (average c-Index) to measure the predictive performance of correctly ranking the predicted patient risk scores with respect to overall survival.
    ...
    if params.seperate_test:
        patient_id_list, test_patient_id = train_test_split(patient_id_list, test_size=0.2, random_state = params.kfold_split_seed)
        seperate_test_dataset = CustomizedDataset(test_patient_id, df_gene, df_clinical, dict_family_genes, params.feature_dimension, params.feature_folder)
        seperate_test_loader = DataLoader(seperate_test_dataset, batch_size = params.batch_size, shuffle=False, num_workers=12)
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(patient_id_list)):
        ...
        if params.less_data:
            ...
            finetune_train_dataset = CustomizedDataset(finetune_train_patient_id, df_gene, df_clinical, dict_family_genes, params.feature_dimension, params.feature_folder)
        ...
        train_dataset = CustomizedDataset(train_patient_id, df_gene, df_clinical, dict_family_genes, params.feature_dimension, params.feature_folder)
        train_loader = DataLoader(train_dataset, batch_size = params.batch_size, shuffle=True, num_workers=12)
        test_dataset = CustomizedDataset(test_patient_id, df_gene, df_clinical, dict_family_genes, params.feature_dimension, params.feature_folder)
    ...
    #     print(x_omics[1].shape)
    #     print(x_omics[2].shape)
    #     print(x_omics[3].shape)
    #     print(x_omics[4].shape)
    #     print(x_omics[5].shape)
    #     print(x_omics[6].shape)
    #     print(x_omics[7].shape)
        # print(cen  sorship)
        ...
        if params.model_type == 'MCAT':
            model = MCAT_Surv_modified(fusion = params.model_fusion_type, omic_sizes=[i.shape[1] for i in x_omics])
        elif params.model_type == 'PathOmics':
            model = PathOmics_Surv(device = device, fusion = params.model_fusion_type, omic_sizes=[i.shape[1] for i in x_omics], model_size_omic = model_size_omic, omic_bag = params.omic_bag_type, use_GAP_in_pretrain = params.use_GAP_in_pretrain_flag, proj_ratio = params.proj_ratio, image_group_method = params.image_group_method)
        ...
        #     device = "cuda" if torch.cuda.is_available() else "cpu"
            ...
            logs, best_pretrain_model = train_and_evaluate(fold, params.pretrain_epochs, model, [train_loader, test_loader], pretrain_optimizer, loss_fn_pretrain, reg_fn, device, save_path, lambda_reg=0., gc = params.gradient_accumulation_steps, save_model = False, seperate_test_mode = params.seperate_test, model_mode = 'pretrain', fold_mode = 'k_fold')
        ...
        print('Finetune')
    #     if params.seperate_test:
    #         test_loader = params.seperate_test_loader
        ...
        params_to_update = []
        not_change = []
        for name,param in best_pretrain_model.named_parameters():
            if param.requires_grad == True:
                params_to_update.append(param)
            else:
                not_change.append(name)

        print(not_change)

        finetune_optimizer = optim.Adam(params_to_update, lr = params.finetune_lr, weight_decay = params.wd_ft)
        #optim.Adam(best_pretrain_model.parameters(), lr = params.finetune_lr, weight_decay = params.wd_ft)
        if params.seperate_test:
            logs, best_test_c_index, best_epoch = train_and_evaluate(fold, params.finetune_epochs, best_pretrain_model, [train_loader, test_loader, seperate_test_loader], finetune_optimizer, loss_fn, reg_fn, device, save_path, lambda_reg=0., gc = params.gradient_accumulation_steps, save_model = True, seperate_test_mode = params.seperate_test, model_mode = 'Finetune', fold_mode = 'k_fold')
        else:
            logs, best_test_c_index, best_epoch = train_and_evaluate(fold, params.finetune_epochs, best_pretrain_model, [train_loader, test_loader], finetune_optimizer, loss_fn, reg_fn, device, save_path, lambda_reg=0., gc = params.gradient_accumulation_steps, save_model = True, seperate_test_mode = params.seperate_test, model_mode = 'Finetune', fold_mode = 'k_fold')
        ...
        if not params.load_model_finetune:
            ...
            if params.model_fusion_type != 'concat':
                save.to_csv(save_path + '/{}_fold{}_Train_test_logs.csv'.format(params.model_fusion_type, fold + 1), header=True)



    # with open(save_path + '/logs_models.json', 'w') as outfile:
    #     json.dump(logs_models, outfile)
    print()
    print()
