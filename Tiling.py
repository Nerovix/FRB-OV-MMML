...
def get_tiles(full_image_path: str, 
    ...
    all=len(x_start_positions)*len(y_start_positions)
    # pbar = tqdm(total = all, desc = 'Processing ...')
    ...
    for x_index, x_start_position in enumerate(x_start_positions):
        for y_index, y_start_position in enumerate(y_start_positions):
            ...
            if len(tiles) == 1000:
                ...
                for tile in tiles:
                    with t.no_grad():
                        ts = totensor(tile).to(device)
                        if not (ts.numel() != 3 * 512 * 512 or (ts > 0.90).sum() > ts.numel() * 0.95):
                            res.append(net(ts.unsqueeze_(dim = 0), 1024))
                tiles = []
            # pbar.update(1)
    # for tile in tqdm(tiles, desc = 'Extracting ...'):
    for tile in tiles:
