import sys
from model_and_training_utils.PathOmics_model_utils import *
import random
...
class PathOmics_Surv(nn.Module):
    def __init__(self, device, fusion='concat', omic_sizes=[100, 200, 300, 400, 500, 600], model_size_wsi: str='small', 
...
#         print("*** 1. Bag-Level Representation (FC Processing) ***")
        ...
        slide_pseudo_feat = []
        numGroup = len(x_omic) if x_omic is not None else 8
    ...
    #             slide_sub_labels.append(tslideLabel)
                subFeat_tensor = torch.index_select(x_path, dim=0, index=torch.LongTensor(tindex).to(self.device))  # n x 1024
...
#         print("*** 4. Global Attention Pooling ***")
    ...
    #         print()
        ...
        if mode == 'pretrain':
            ...
            assert self.fusion == 'concat','we use concat'

            if self.fusion == 'bilinear':
                h = self.mm(h_path.unsqueeze(dim=0), h_omic.unsqueeze(dim=0)).squeeze()
            if self.fusion == 'concat':
                if h_path is not None and h_omic is not None:
                    h = self.mm(torch.cat([h_path, h_omic], axis=0))
                elif h_path is None:
                    h = self.mm(torch.cat([h_omic, h_omic], axis=0))
                elif h_omic is None:
                    h = self.mm(torch.cat([h_path, h_path], axis=0))
...
class MultiheadAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None):
        ...
        self.out_proj = NonDynamicallyQuantizableLinear(embed_dim, embed_dim)#_LinearWithBias(embed_dim, embed_dim)#
