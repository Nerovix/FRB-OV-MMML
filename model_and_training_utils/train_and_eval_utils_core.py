            ...
            if not seperate_test_mode:
                ...
                if val_c_index > best_val_c_index:
                    ...
                    if test_c_index > best_test_c_index:
                        ...
                        best_test_c_index = test_c_index
                        best_epoch = epoch
                        print("Save a new modelllllll")
                        if save_model:
                            torch.save({
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    }, save_path + '/' + 'fold_{}_finetune_model_{}.pt'.format(fold, epoch))
                            print('saved to '+save_path + '/' + 'fold_{}_finetune_model_{}.pt'.format(fold, epoch))
                        logs['test_loss'].append(test_loss)
                        logs['test_c_index'].append(test_c_index)
                    else:
                        logs['test_loss'].append(0)
...
#    adjust_lr(optimizer, epoch, num_epochs, init_lr)
    ...
    for batch_idx,(data_WSI, data_omic, c, event_time, label, patient_id) in enumerate((loader)):
...
#             loss += loss_mse
            ...
            # Y_hat就是想要的答案，是一个torch.tensor([[0/1/2/3]])
            # print(Y_hat)
